...
def test_single(model, test_loader, mode='2'):
    model.eval()
    test_matrix_teacher = np.zeros([(colormap), (colormap)])
    loop = tqdm(test_loader, leave=True)
    with torch.no_grad():
        for data1, data2, target,_ in loop:
            data1, data2, target = data1.cuda(), data2.cuda(), target.cuda()
            data_all=IHS(data1,data2)
            output = model(data_all)
            pred = output.data.max(1, keepdim=True)[1]
            for i in range(len(target)):
                test_matrix_teacher[int(pred[i].item())][int(target[i].item())] += 1
            loop.set_postfix(mode='test')
        loop.close()
    return test_matrix_teacher
...
if __name__=='__main__':
    Student1_path='/root/autodl-tmp/Huhehaote/pre trained models/studentpan.pkl'
    Student2_path='/root/autodl-tmp/Huhehaote/pre trained models/studentms.pkl'
    teacher_path='/root/autodl-tmp/Huhehaote/pre trained models/teacher.pkl'
    Student1=torch.load(Student1_path).cuda()
    Student2=torch.load(Student2_path).cuda()
    Teacher=torch.load(teacher_path).cuda()
    train_loader,test_loader,unlabeled_loader=getDataset()
    matrix_pan,matrix_ms=test_second(Student1,Student2,test_loader)
    matrix_t=test_single(Teacher,test_loader)
    print("t's OA, AA as follow")
    aa_oa(matrix_t)
    # print(matrix_pan)
    # print(matrix_ms)
