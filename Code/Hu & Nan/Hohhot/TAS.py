...
#主要任务：过滤出来数据，给G-SN学习
    ...
    # loading model
    teacher=torch.load("./Version2/pre trained models/teacher.pkl").cuda()
    # Dataset
    ...
    #hypara
    # EPO=200
    # TAE=Test_Teacher(teacher,test_loader)
    # th=0.1
    # DI=GenDI(th,type=0)
    # for epo in range(1,EPO+1):
    #     print("epoch is {}".format(epo))
    #     Acc_1,Acc_2=Test_Sudents(student1_model=architecture.Student1.cuda(),student2_model=architecture.Student2.cuda(),
    #                              test_loader=test_loader,TAE=TAE)
    #     S_t=(1/2)*(Acc_1+Acc_2)
    #     if epo==1:
    #         pass
    #     else:
    #         TAE=updata_TAE(DI,epo,EPO,BaseLine=0.9,S_t=S_t,TAE=TAE)
    #     S=GenS(Acc_1,Acc_2,TAE)
    #     th,flag=updata_th(th,epo,EPO,S)
    #     filter_loader=KLP(teacher_model=teacher,training_data=train_loader,th=th)
    #     if flag==1:
    #         pass
    #     else:
    #         pass
    ...
    # for ms,pan,label in filter_loader:
    #     print(ms.size())
    #     print(pan.size())
    #     print(label.size())
    # print(FD1.ms_patch.size())
    # print(FD1.pan_patch.size())
    # print(FD1.label.size())
    # filter_loader = DataLoader(dataset=FD1, batch_size=BATCH_SIZE, shuffle=False, num_workers=0)
